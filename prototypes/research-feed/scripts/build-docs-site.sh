...
DOCS=(
  ...
  "identity-verification-design|$REPO_ROOT/docs/2026-07-23-identity-verification-design.md|Identity Verification — Design & Threat Model|Companion to EPIC-A: why a registration number is a public claim not a secret, the HCPC register findings, the critical ID-name↔register name-match binding, explicit user flow + result paths, and the IDV provider recommendation.|Design deep-dives"
  "research-feed-ingestion|$REPO_ROOT/docs/2026-08-03-research-feed-ingestion-design.md|Research Feed — Ingestion, Classification & Scoring (S8a)|Companion to EPIC-I: why the prototype has to be turned inside out, the split between precomputed article scores and per-member topic match, domain-bounded corpus queries, the 588-tag false-positive risk, and what still needs deciding.|Design deep-dives"
  "post-follow|$REPO_ROOT/docs/2026-08-06-post-follow-design.md|Follow a Discussion (S15)|Companion to EPIC-B §8 and EPIC-G: why thread-following comes before the rest of S7, the defect it fixes in today's reply notifications, the amendment retiring tag-follows in favour of member interests, and the collapsing upsert that stops a busy thread flooding the inbox.|Design deep-dives"
  "style-guide|$REPO_ROOT/docs/style-guide/STYLE_GUIDE.md|Design System & Style Guide|The canonical look: colour tokens, typography (Fraunces + Inter), spacing, components, WCAG 2.2 AA rules, voice, and the anonymity/safety UX patterns. Values live in packages/design-tokens and generate the web CSS layer.|Design deep-dives"
