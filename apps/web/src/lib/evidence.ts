 ...
 *
 * **It lives apart from any control that uses it**, and that is the whole point of the file.
 * It began as an export of the `'use client'` component that rendered the picker — and every
 * export of a client module becomes a *client reference* when a server component imports it,
 * not the value itself. The page's `EVIDENCE_TYPES.includes(...)` therefore type-checked,
 * built cleanly, and threw `EVIDENCE_TYPES.includes is not a function` on the first real
 * render.
 *
