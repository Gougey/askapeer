...
## S18 — Clinical thesaurus for free-text search  [parallel-able] [Should] — **S18b built 2026-09-01**
...
- **Delivers (S18b)** — **BUILT 2026-09-01**, migration `0031_seed_part2_synonyms.sql`, generated by `docs/tools/vocabulary/synonyms.py`. 90 of Part 2's 349 preferred terms name a tag; tags carrying synonyms go **11 → 105** and distinct synonym terms **27 → 139**. Twenty-three aliases are deliberately withheld — two footnoted in the source, two meaning more than one tag, seven that are themselves tags (a merge question for Andrew), and **fourteen bare abbreviations** (`oa`, `as`, `ra`, `psa`, `ocd`…) which would enter the classifier unguarded, since its ambiguity guard covers tag names only. The abbreviations are unproven rather than rejected: the admin tag screen's dry run is exactly the evidence needed to add them one at a time. All are listed in Appendix B. Existing synonyms are unioned, never overwritten, so 0025's hand-tuned eleven stand.
...
- **Depends on**: S16 for the thesaurus to cover both corpora. S18b depended on nothing and went first.
- **The Part 1 taxonomy expansion landed with it** — migration `0030_expand_clinical_taxonomy.sql`, taking the taxonomy 588 → 1,304 tags: the ligaments axis and the pelvis region, neither of which existed, plus the parent levels the v2.0 seed flattened. It is not a re-seed; every existing id survives, 126 tags are re-parented rather than duplicated, and the flattened groups are retired rather than deleted. It barely moves S18b's numbers (85 → 90 matchable terms) because Part 2 is conditions phrased differently, not the anatomy Part 1 added. **One consequence to watch**: `GET /v1/tags` now returns a 237 kB payload where `vocabulary.service.ts` reckoned ~600 rows was the comfortable limit for a single call — the `?prefix=` typeahead EPIC-C §5 specifies is now worth its keep.
