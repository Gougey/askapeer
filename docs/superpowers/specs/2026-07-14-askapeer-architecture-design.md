...
- **Web app** — Next.js (React), the sole MVP client, communicating with the API entirely over HTTPS. Built API-first (no server-only logic baked into the web app) so a future native mobile client is architecturally just another API consumer, not a rewrite. **Internationalisation (added 2026-07-19, gap G-10)**: all user-facing strings are externalised from day one into **per-locale message catalogs** (keyed message ids, resource files) via a standard i18n library — never hardcoded literals — with an `en-GB` catalog at MVP and locale-aware formatting (dates, `£`, age-bands). This is *translation-readiness*, not multi-language delivery (which stays Won't-have per PRD §6.1); it is **not** a database "language table" (UI copy is versioned with the code). Retrofitting i18n later is expensive; adopting the discipline now is nearly free.
...
Session mechanics: a short-lived access token (around 15 minutes) plus a rotating refresh token. The web app stores the refresh token in an HttpOnly secure cookie (mitigates cross-site scripting attacks); a future native mobile client would receive it in the response body to store in platform-native secure storage instead — the same endpoints serve both, so no backend change is needed when mobile ships. The refresh token rotates on each use (reuse-detection catches theft), so the magic link is a *one-time bootstrap* — members stay signed in across app opens via the refresh token, not by re-clicking a link.

**Refresh-token lifetime (amended 2026-07-19, gap G-11)**: the persistent session is bounded by a **sliding inactivity window plus an absolute maximum** — the single parameter that determines how often members re-authenticate. Both are **tunable settings** in EPIC-J config (`auth.session_inactivity_days`, `auth.session_absolute_max_days`) rather than hardcoded, so they can be adjusted from real usage without a deploy. Concrete defaults are a value to set before build (a decision, not a mechanism).

**Passkeys — biometric sign-in (amended 2026-07-19, gap G-9)**: an *additional* passwordless method alongside the magic link. After first sign-in a member may register a **WebAuthn platform authenticator** (Face ID / Touch ID / Android biometric / Windows Hello); subsequent sign-ins (when the session has ended) use the passkey instead of a fresh magic link. Passkeys are phishing-resistant, first-party, per-device, and carry unchanged to native. Adds `identity.webauthn_credentials (member_id, credential_id, public_key, sign_count, transports, device_label, created_at, last_used_at)` and WebAuthn **registration** + **assertion** endpoints under `IdentityService`. **Build-timing — resolved 2026-07-19 (Adrian): fast-follow** (early Phase 1.x), not the initial release — the magic-link + persistent session is sufficient to launch, and re-auth is infrequent, so passkeys land shortly after rather than gating launch. An optional biometric **app-lock** (gate every app open even mid-session) is **offered as an opt-in setting, off by default** (resolved 2026-07-19) — shared-device privacy without forcing friction; managed on the sign-in & security screen, meaningful once passkeys exist.
