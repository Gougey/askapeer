...
| Tag vocabulary | `community.tags` | EPIC-C | Add, merge (fold one tag's posts into another), **retire** — pending FD-4 closure |
| Search synonym dictionary | Postgres text-search config + a synonyms table | EPIC-C Section 4 | Maintain synonym entries (seeded from the tag→MeSH mapping — see `docs/2026-07-17-taxonomy-standards-research.md`); pending FD-4 |
...
**Tag merge** exists because free tagging (FD-4's hybrid model) inevitably produces near-duplicates (`acl` / `acl-tear` / `anterior-cruciate-ligament`). Merge repoints the losing tag's `post_tags` rows to the winning tag and retires the loser — an Administrator cleanup action, audited like any other.
