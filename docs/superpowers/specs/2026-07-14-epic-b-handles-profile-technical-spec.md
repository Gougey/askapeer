...
- **An expelled member re-registers under EPIC-A with the same professional registration**: EPIC-A's uniqueness constraint (its spec, Section 2) only blocks non-`rejected` prior members, and expulsion doesn't change `identity.members.verification_status` to `rejected` — this is a gap between the two specs worth resolving explicitly (Section 13), since as written a permanently expelled practitioner could plausibly re-verify and create a brand-new handle, which seems to contradict "immediate permanent expulsion."
...
- **Expelled-member re-registration gap** (Section 10): EPIC-A's uniqueness constraint doesn't currently prevent a genuinely expelled practitioner from re-verifying under the same registration and creating a new handle, since expulsion doesn't set `verification_status = rejected`. Needs a cross-epic decision: either expulsion should also flip the `identity.members` status, or EPIC-A's registration check needs to consult moderation history, not just verification history. Worth resolving before EPIC-F is written, since EPIC-F is what actually performs expulsions.
