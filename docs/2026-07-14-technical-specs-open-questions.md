...
- **Push channel — per-type triggers and throttling** (deferred to testing, not blocking): the push mechanism/preference is now specified and ships inert for MVP (see §2) — which notification types actually push, and the batching/throttling to avoid nuisance, are to be settled during app testing. Also a prerequisite check: the web client must be a PWA with a service worker (and, on iOS, added-to-Home-Screen) before push can be switched on.

*(The tag-follow mechanism the digest depends on is resolved — see §2. The push channel addition itself is resolved — see §2.)*
