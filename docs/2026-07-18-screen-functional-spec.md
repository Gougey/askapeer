...
- **App-lock — optional, a separate privacy feature (RESOLVED 2026-07-19: offer it, opt-in & off by default).** A biometric gate on *every* app open, even while the session is valid, for shared-device privacy — independent of session validity. Available in Settings → Sign-in & security (F8), off by default so it adds no friction for everyone else; meaningful once passkeys exist.
...
- **Build-timing — resolved 2026-07-19 (Adrian): fast-follow** (early Phase 1.x), not the initial release. Magic-link + persistent session ship at launch; passkeys land shortly after (re-auth is infrequent, so the wait is painless). Mechanism per **G-9** (EPIC-A / architecture §5.2: WebAuthn register/assert endpoints + `identity.webauthn_credentials`). The A9 permission screen + F8 management screen exist in the design but activate when the passkey feature ships.
...
- **Content**: **category** picker (content-type: Clinical Case / Research / Career / Equipment / General); **tags** — *selected from the curated vocabulary*, facet-grouped, multi-select (members cannot create new tags — EPIC-C §3); **title**; **body**; **optional poll** (attach question + options); the **zero-tolerance anonymity reminder** (mandated §1.4); post.
  ...
  *Anonymity-reminder placement (revised 2026-07-28)*: split rather than a single standing block. A **one-line** reminder sits above the fields, so it is still read before anything is typed; the **full** warning appears in a confirmation gate opened by "Post question", offering **Post** or **Go back and edit**. The standing block cost roughly a fifth of a phone screen and, being unchanging, stopped being read — this keeps the rule present while composing *and* makes it unavoidable at the moment of commitment, which is the last point disclosure can be prevented. The gate is a **reminder, not an attestation**: nothing is recorded. The recorded, identity-linked attestation remains EPIC-E's and applies only to case discussions (D3).
...
**Decision-gaps — resolved 2026-07-19 (Adrian):** **G-9** biometric = **fast-follow** (Phase 1.x); **app-lock** = **offer, opt-in & off**; **G-13** = **record** the acknowledgement (`anonymity_acknowledged_at`); **G-22** saves = **deferred post-launch** (shape specced); **G-15** = seed members get a **grace window** at paywall flip (flip-*timing* remains a business/launch call); **G-20** = **working design adopted** (arch erasure default), exact retention **legal-gated**. No decision-gaps remain open except the two external dependencies (G-15 flip date; G-20 legal retention specifics).
...
4. ~~**Resolve the open decisions**~~ — **done 2026-07-19** (§7): biometric fast-follow, app-lock opt-in/off, record acknowledgement, defer saves, seed grace-window, erasure working-design. Only two external dependencies remain (G-15 flip *date* → launch/business; G-20 retention specifics → legal).
5. **Derive the tracer-bullet slice backlog** from the completed inventory + flows (slice 1 = A1→A5, the onboarding spine) — the natural next step now that specs + screens + gaps + decisions are all settled.
